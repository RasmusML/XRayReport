...
path = "./data/raw"
raw = pd.read_pickle(f'{path}/all_reports_df.pkl')
# vocab = build_vocab(asd.processed_captions.values, 8)
raw = raw[ raw['processed_findings'].notnull() ]
vocab = build_vocab(raw.processed_findings.values, 8)
...
class iuxray(Dataset):
    def __init__(self, vocab = vocab, transform=None):
        self.raw_data = pd.read_pickle(f'{path}/all_reports_df.pkl')
        print(self.raw_data)
        self.raw_data = self.raw_data[ self.raw_data['processed_findings'].notnull() ]
        self.captions_dict = {}
        
        for image, caption in zip(self.raw_data.images, self.raw_data.processed_findings):
            self.captions_dict[image+'.png'] = caption

        self.vocab = vocab
        self.transform = transform
        self.all_imgs = raw.images.values
...
def get_loader(transform, batch_size, shuffle, num_workers, vocab = vocab):
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    # dataset
    data = iuxray(vocab = vocab, transform = transform)
